...
class BasicSemanticMatcher:
    """Keyword-based semantic matching without external API."""
    ...
    def calculate_similarity(self, query: str, target: str) -> float:
        """Compute Jaccard similarity between query and target."""
...
class OpenAISemanticMatcher:
    """OpenAI-powered semantic matching with embeddings."""
...
# ==========================================
# 3. Excel Loader (FULLY IMPLEMENTED)
# ==========================================

class ExcelLoader:
    """Load Excel sheets with formatting metadata."""
    
    def __init__(self, file_path: str):
        """Initialize loader with workbook."""
        self.wb = load_workbook(file_path, data_only=True)
        self.file_path = file_path

    def load_sheet(self, sheet_name: str) -> Tuple[pd.DataFrame, List[List[bool]]]:
        """Load sheet data and bold formatting grid."""
...
# ==========================================
# 4. Table Detector (FULLY IMPLEMENTED)
# ==========================================
...
class TableDetector:
    ...
    def _create_occupancy_grid(self, df: pd.DataFrame, bold_grid: List[List[bool]]) -> np.ndarray:
        """Create binary grid: 1=relevant cell, 0=empty."""
        rows, cols = df.shape
    ...
    def _is_relevant_cell(self, val: Any) -> bool:
        """Check if cell is data-relevant."""
        if pd.isna(val):
            return False
    ...
    def _flood_fill(self, occupancy: np.ndarray, seen: np.ndarray, start_r: int, start_c: int) -> Tuple[int, int, int, int]:
        """Flood fill to find bounding box of component."""
    ...
    def _refine_table(self, df: pd.DataFrame, bold_grid: List[List[bool]], bounds: Tuple[int,int,int,int]) -> TableRegion:
        """Refine table boundaries and detect headers."""
    ...
    def _detect_header_rows(self, df: pd.DataFrame, bold_grid: List[List[bool]], top: int, left: int, right: int) -> int:
        """Detect number of header rows by text ratio and boldness."""
        max_header_rows = 5
        best_rows = 1
    ...
    def _detect_row_header_col(self, df: pd.DataFrame, data_top: int, data_bottom: int, left: int, right: int) -> int:
        """Detect column with row labels (mostly strings)."""
        best_col = left
        best_text_ratio = 0
        for c in range(left, min(right+1, len(df.columns))):
...
# ==========================================
# 5. Header Reconstruction (FULLY IMPLEMENTED)
# ==========================================

class HeaderReconstructor:
    """Reconstruct hierarchical column headers."""
    
    def reconstruct_column_headers(self, df: pd.DataFrame, table: TableRegion) -> List[List[str]]:
        """Build multi-level header paths for each column."""
        ...
        # Forward fill horizontally (left to right)
        ...
        # Backward fill horizontally (right to left)
        for r_idx in range(len(header_data)):
            next_val = None
...
# ==========================================
# 6. Cell Classifier (FULLY IMPLEMENTED)
# ==========================================

class CellClassifier:
    """Classify cells into roles."""
    ...
    def classify(self, df: pd.DataFrame, table: TableRegion, bold_grid: List[List[bool]]) -> List[CellClassification]:
        ...
        for r in range(table.top_row, table.bottom_row+1):
            for c in range(table.left_col, table.right_col+1):
                ...
                role = self._classify_cell_role(r, c, val, in_header, is_row_header_col, is_bold)
                confidence = 1.0 if role != 'empty' else 0.0
                classifications.append(CellClassification(r, c, val, role, confidence, is_bold))
    ...
    def _classify_cell_role(self, row: int, col: int, val: Any, in_header: bool, is_row_header_col: bool, is_bold: bool) -> str:
        ...
        if self._is_numeric_like(val):
            return 'data_value'
        if isinstance(val, str) and len(str(val).split()) > 4:
            return 'annotation'
        return 'data_value' if is_bold else 'annotation'

    def _is_numeric_like(self, val: Any) -> bool:
        """Check if value is numeric or numeric-like."""
        if pd.isna(val):
            return False
        if isinstance(val, (int, float)):
            return True
...
# ==========================================
# 7. Data Indexer (FULLY IMPLEMENTED)
# ==========================================

class EnterpriseDataIndexer:
    """Build searchable index from tables."""
    ...
    def build_index(self, df: pd.DataFrame, bold_grid: List[List[bool]], tables: List[TableRegion]):
        """Build complete index from tables."""
...
# ==========================================
# 8. Main Engine (FULLY IMPLEMENTED)
# ==========================================

class FinancialExcelEngineV7:
    """Complete financial Excel extraction engine."""
    ...
    def __init__(self, filepath: str, config: QueryEngineConfig = QueryEngineConfig()):
        ...
        print(f"✅ Engine ready: {len(self.indexer.records)} data points indexed")

    def _load_and_index(self):
        """Load all sheets and build index."""
        ...
        for sheetname in wb.sheetnames:
            try:
                ...
                print(f"⚠️  Warning: Error processing sheet {sheetname}: {e}")

    def query(self, question: str, top_k: int = 5) -> List[SearchResult]:
        """Search for data matching question."""
...
if __name__ == "__main__":
    ...
    # Initialize engine
    config = QueryEngineConfig(semantic_backend='basic', min_table_density=0.3, min_confidence=0.3)
    ...
    print("\n" + "="*70)
    print("✅ ALL TESTS COMPLETE - Engine V7 fully operational!")
    print("="*70)
